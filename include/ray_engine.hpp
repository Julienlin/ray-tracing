 ...
 * @file ray_engine.hpp
 * @author Julien Lin
 * @brief This file contains the declarations of RayEngine class and
 * RayCastingEngine class.
 * @version 0.1
 * @date 2020-02-09
 *
...
/**
 * @brief This class is the motherclass of ray engine class. This class can be
 * used as an interface for ray engines.
 * @class RayEngine.
 */
class RayEngine {
protected:
...
public:
  ...
  /**
   * @brief This type define the relation between a light source and the
  ...
  /**
   * @brief This type define the relation between a incident ray, an object and
   * the inetsection point.
   *
   */
  ...
  RayEngine(std::vector<Ray> &rays, std::vector<SceneBaseObject *> &objects,
            ...
            position_t &observer_pos, RGBColor background_color = RGB_BLACK,
            double amb_lighting = 1, int deepth = 1);
  ...
  RayEngine(std::vector<SceneBaseObject *> &objects,
            std::vector<LightSource> &sources, Screen &screen,
            position_t &obs_pos, RGBColor background_color = RGB_BLACK,
            double amb_lighting = 1, int deepth = 1);

  /**
   * @brief Construct a new Ray Engine object. This constructor is generated by
   * the compiler.
   *
   * @param engine
   */
  ...
  /**
   * @brief This function launch the rendering of the image. This function is
   * the interface from which we can launch every ray engine.
   *
   */
...
public:
  ...
  RayCastingEngine(std::vector<SceneBaseObject *> &objects,
                   ...
                   RGBColor background_color = RGB_BLACK,
                   double amb_lighting = 1);

  /**
   * @brief Construct a new Ray Casting Engine object. This constructor is
   ...
   *
   */
  void get_reachable_sources(Ray &, SceneBaseObject *, position_t,
                             std::vector<source_vect_t> &);
  ...
  /**
   * @brief Compute the color seen given an intersection a source and a
   * reflected ray.
   *
   ...
   * @return RGBColor
   */
  RGBColor compute_color(ray_obj_dist_t &dist, source_vect_t &source,
                         Ray &reflected);

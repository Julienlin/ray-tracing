...
Screen::Screen(int H, int W, double pix_size, vector_t v_W, vector_t v_H,
    ...
    : m_H(H), m_W(W), m_pix_size(pix_size), m_v_H(v_H), m_v_W(v_W),
      ...
      m_crossing_rays(m_H * m_W), m_normal((v_W ^ v_W).normalize()) {
  // int n = m_H * m_W;
  ...
  for (int i = 0; i < m_H; i++) {
    ...
    for (int j = 0; j < m_W; j++) {
      ...
      if (!(i == 0 && j == 0)) {
        ...
        // std::cout << "m_pix_pos[" << i * m_W + j << "] : " << m_pix_pos[i *
        // m_W + j] << std::endl;
        cur_v_W += m_v_W;
      }
    }
...
bool Screen::is_intersecting(Ray &ray) {
  ...
  if (ray_direction * m_normal == 0) {
    if ((ray_pos - m_screen_pos) * m_normal ==
        0) { // the line is in the plane the screen. We now
      // do nothing.
      return false;
    }
    return false;
  ...
  double t =
      (m_normal * (ray_pos - m_screen_pos)) / ((-ray_direction) * m_normal);
  double u =
      ((m_v_W ^ (-ray_direction)).normalize() * (ray_pos - m_screen_pos)) /
      ((-ray_direction) * m_normal);
  double v =
      (((-ray_direction) ^ m_v_H).normalize() * (ray_pos - m_screen_pos)) /
      ((-ray_direction) * m_normal);
...
position_t Screen::intersect_pt(Ray &ray) {
  ...
  double t =
      (m_normal * (ray_pos - m_screen_pos)) / ((-ray_direction) * m_normal);
  double u = ((m_v_W ^ (-ray_direction)).normalize() * (ray_pos - m_screen_pos)) /
             ((-ray_direction) * m_normal);
  double v = (((-ray_direction) ^ m_v_H).normalize() * (ray_pos - m_screen_pos)) /
             ((-ray_direction) * m_normal);
